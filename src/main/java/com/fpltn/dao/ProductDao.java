...
public class ProductDao {
	...
	public static List<Product> sortProductasc() {
		try (Session session = HibernateUtil.getSessionFactory().openSession()) {
			return session.createQuery(" from Product order by gia asc", Product.class).list();
		}
	}

	public static List<Product> sortProductdesc() {
		try (Session session = HibernateUtil.getSessionFactory().openSession()) {
			return session.createQuery(" from Product order by gia desc", Product.class).list();
		}
	}

	public static List<Product> getProductbyName(String name) {
		try (Session session = HibernateUtil.getSessionFactory().openSession()) {
			return session.createQuery(" from Product where name like :tukhoa", Product.class)
					.setParameter("tukhoa", "%" + name + "%").list();
		}
	}

	public static List<Product> getProductbyNguoitao(int name) {
		try (Session session = HibernateUtil.getSessionFactory().openSession()) {
			return session.createQuery(" from Product where nguoitao = :tukhoa", Product.class)
					.setParameter("tukhoa", name).list();
		}
	}
	...
	public static List<Product> getProductbyAccount(int acc) {
		...
		try (Session session = HibernateUtil.getSessionFactory().openSession()) {
			return session.createQuery("from Product where nguoitao = :pro", Product.class).setParameter("pro", pro)
					.list();
		}
	}
